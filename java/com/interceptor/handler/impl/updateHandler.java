 ...
 */
@Component
@CommandType(value = SqlCommandType.UPDATE)
public class updateHandler implements SqlCommandTypeExecutorHandler {
    ...
    public Object doExecutorHandler(Invocation invocation) throws SQLException {
        ...
        for (SimonInterceptorHandler handler : handlers) {
            //便利构建实体类
            try {
                parameter = handler.updateBefore(mappedStatement, parameter, mappedStatement.getBoundSql(parameter));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
